...
Acl.prototype.allow = function(role, resource, actions, assertion) {
  ...
  for (var i in actions)
    this.permissions.push(new Permission(role || null, resource || null, actions[i] || null, assertion || true, PermisionType.ALLOW));
};

Acl.prototype.deny = function(role, resource, actions, assertion) {
  if (!isArray(actions)) actions = [actions];
  for (var i in actions)
    this.permissions.push(new Permission(role || null, resource || null, actions[i] || null, assertion || false, PermisionType.DENY));
};
